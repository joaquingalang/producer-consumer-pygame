...
box_small = BoxSmall('1')
box_large = BoxLarge('1')

# Draw Producer & Consumer Counters
def draw_counters(screen, prod_count, con_count):
    producer_counter = CounterBox('Producers:', prod_count, '#F5C5C5', COUNTER_RECT_SIZE, ((SCREEN_WIDTH // 2) - 350, 375))
    screen.blit(producer_counter.image, producer_counter.rect)

    consumer_counter = CounterBox('Consumers:', con_count, '#F5C5C5', COUNTER_RECT_SIZE, ((SCREEN_WIDTH // 2) + 50, 375))
    screen.blit(consumer_counter.image, consumer_counter.rect)

    # Producer & Consumer Controller Divider
    pygame.draw.line(screen, (0, 0, 0), (SCREEN_WIDTH // 2, 380), (SCREEN_WIDTH // 2, 420), 4)
...
def display_elements(screen, producers, consumers, buffer):
    ...
    for index, value in enumerate(buffer):
        ...
        box.rect.topleft = (index * 89 + 100, 250)
        box.draw(screen)
